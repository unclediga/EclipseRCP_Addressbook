...
public class SearchComposite extends Composite {
	...
	public SearchComposite(Composite parent) {
		...
		int columns = 1;
		/*
		 * BORO if (!CrossPlatformSupport.isStyleSupported(SWT.ICON_SEARCH)) {
		 * Label searchLabel = new Label(this, SWT.NONE);
		 * searchLabel.setImage(resources
		 * .createImage(AddressBook.ICON_MAGNIFIER));
		 * GridDataFactory.fillDefaults().align(SWT.LEFT,
		 * SWT.CENTER).applyTo(searchLabel); columns++; }
		 */
